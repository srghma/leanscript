...
/--
info: TyTests.NominalGetCtorTest.Option.none.leanScriptCtor {ks : List ℕ} {Δ : DSig ks} {Γ : Ctx ks} (α : Ty ks) :
...
/--
info: TyTests.NominalGetCtorTest.Prod.mk.leanScriptCtor {ks : List ℕ} {Δ : DSig ks} {Γ : Ctx ks} (α β : Ty ks)
  (x0 : Term Δ Γ α) (x1 : Term Δ Γ β) : Term Δ Γ (α.record (Fields.one β))
-/
#guard_msgs in
...
/--
info: TyTests.NominalGetCtorTest.Option.leanScriptCases {ks : List ℕ} {Δ : DSig ks} {Γ : Ctx ks} (α : Ty ks) {τ : Ty ks}
...
error: LeanScript: the recursive type
  Other
is not declared in the signature `NominalGetCtorTest.Prog`; add it to `leanscript_signature NominalGetCtorTest.Prog`
-/
#guard_msgs in
...
/--
error: LeanScript: `ℕ` is a leaf of the language: its values are literals, not constructor applications
-/
#guard_msgs in
