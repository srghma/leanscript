...
| `Ev.fib` (over a family whose members mention **each other**) | `mutualRecursiveFamily_rec 1`, descending into the *other member* | `evFibTerm` |
| `Node.fib` (over a family of a record, a union and a newtype) | `mutualRecursiveFamily_rec 1`, both recursive members descending | `nodeFibTerm` |
...
that is the member whose constructor the fold is walking down; §7 descends into the
*other* member. -/
...
/-! ## 7. A deeper look into the **other** member

The family above is a `mutual` block whose members do not mention each other, so every
descent stayed inside the member it started in.  A family whose members are defined *in
...
/-! ## 8. The other two member shapes: a record member and a newtype member

The two families above are made of `ctors` members.  A member of a family can also be a
...
/-! ## 9. What the evaluator says about these terms
...
/-! ## 10. A depth is needed: what cannot be written without one
