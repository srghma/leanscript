...
`TyTests/ArrayRecDepthTest.lean`: `cont` at depth one, and `cont3`, `cont4` at depths two and
three.  The overview of the suite is in `TyTests/ArrayRecDepthTest.lean`.
-/
