...
/-- Running a closed term of `emptySig`. -/
abbrev run {τ : Ty} (t : Term emptySig [] τ) : Ty.Den τ := Term.run GlobalEnv.nil t
...
/-- `fun x => fun y => x`. -/
def constNat : Term emptySig [] (Ty.prim .nat ⇒ Ty.prim .bool ⇒ Ty.prim .nat) :=
  .lam (.lam (.var (v♯1)))
...
def callDouble : Term doubleSig [] (Ty.prim .nat) := .ap (.global .here) (.nat_mk 21)
...
def foldNatZero : Term emptySig [] (Ty.prim .nat ⇒ Ty.prim .nat) :=
...
def foldNatPred : Term emptySig [] (Ty.prim .nat ⇒ Ty.prim .nat) :=
  .lam (.nat_rec (.var (v♯0)) (.nat_mk 0) (.var (v♯0)))

/-- `fun i => match i with | .ofNat n => n | .negSucc n => n`. -/
def intMagnitude : Term emptySig [] (Ty.prim .int ⇒ Ty.prim .nat) :=
...
def uint8Bits : Term emptySig [] (Ty.prim .uint8 ⇒ Ty.prim (.bitvec 8)) :=
...
def foldArrayZero : Term emptySig [] (Ty.array (Ty.prim .nat) ⇒ Ty.prim .nat) :=
...
def lastOrZero : Term emptySig [] (Ty.array (Ty.prim .nat) ⇒ Ty.prim .nat) :=
...
/-- Its first field. -/
def pairFst : Term emptySig [] (Ty.prim .nat) := .record_casesOn pair (.var (v♯0))

/-- Its second field. -/
def pairSnd : Term emptySig [] (Ty.prim .bool) := .record_casesOn pair (.var (v♯1))
...
def nothing' : Term emptySig [] (Ty.taggedUnion natOrNothing) :=
  .taggedUnion_mk natOrNothing 0 (fields := .nil)

/-- Its constructor that carries a `nat`, applied to `5`. -/
def justFive : Term emptySig [] (Ty.taggedUnion natOrNothing) :=
...
/-- One branch per constructor, with no default: the middle constructor's branch binds
    its `bool`. -/
def natBoolNatAll : Term emptySig [] (Ty.taggedUnion natBoolNat ⇒ Ty.prim .nat) :=
