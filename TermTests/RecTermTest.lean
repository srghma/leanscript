...
def natNil : SomeTerm recEmptySig [] natListTy :=
  ⟨.recTaggedUnion_mk natListSchema (t := 0) (fields := .nil)⟩

/-- `[3]`: `cons` of `3` and the empty list. -/
def natOne : SomeTerm recEmptySig [] natListTy :=
  ⟨.recTaggedUnion_mk natListSchema (t := 1)
    (fields := .cons (.nat_mk 3) (.cons natNil.term .nil))⟩
...
def natHead : SomeTerm recEmptySig [] (natListTy ⇒ TyWf.prim .nat) :=
...
def natTail : SomeTerm recEmptySig [] (natListTy ⇒ natListTy) :=
...
def natFoldZero : SomeTerm recEmptySig [] (natListTy ⇒ TyWf.prim .nat) :=
...
def roseLeaf : SomeTerm recEmptySig [] roseTy :=
  ⟨.recObject_mk roseSchema (fields := .cons (.nat_mk 1) (.cons (.array_mk .nil) .nil))⟩

/-- A tree with one child. -/
def roseOne : SomeTerm recEmptySig [] roseTy :=
  ⟨.recObject_mk roseSchema
    (fields := .cons (.nat_mk 2) (.cons (.array_mk (.cons roseLeaf.term .nil)) .nil))⟩

/-- The label of a tree: the eliminator binds every field, so the label is index `0`. -/
def roseLabel : SomeTerm recEmptySig [] (roseTy ⇒ TyWf.prim .nat) :=
...
def emptyForest : SomeTerm recEmptySig [] forestTy :=
  ⟨.recAlias_mk (Ty.array Ty.self).toTyWfIn (value := .array_mk .nil)⟩

/-- A forest of one empty forest. -/
def oneForest : SomeTerm recEmptySig [] forestTy :=
  ⟨.recAlias_mk (Ty.array Ty.self).toTyWfIn (value := .array_mk (.cons emptyForest.term .nil))⟩

/-- Is a forest empty?  The eliminator binds the body, which is an array. -/
def forestIsEmpty : SomeTerm recEmptySig [] (forestTy ⇒ TyWf.prim .bool) :=
...
def aNil : SomeTerm recEmptySig [] tyA :=
  ⟨.mutualRecursiveFamily_mk famA (value := .ctors _ 0 (fields := .nil))⟩

/-- A `B`: the natural `7` and the `A` above. -/
def bOne : SomeTerm recEmptySig [] tyB :=
  ⟨.mutualRecursiveFamily_mk famB
    (value := .record _ (.cons (.nat_mk 7) (.cons aNil.term .nil)))⟩

/-- An `A` built from that `B`. -/
def aOne : SomeTerm recEmptySig [] tyA :=
  ⟨.mutualRecursiveFamily_mk famA (value := .ctors _ 1 (fields := .cons bOne.term .nil))⟩

/-- The natural a `B` holds: a record member has one branch, which binds its fields. -/
def bLabel : SomeTerm recEmptySig [] (tyB ⇒ TyWf.prim .nat) :=
...
/-- Is an `A` the field-less constructor?  A dispatch on every constructor of the member
    `A` is. -/
def aIsNil : SomeTerm recEmptySig [] (tyA ⇒ TyWf.prim .bool) :=
...
def famFold : SomeTerm recEmptySig [] (tyA ⇒ TyWf.prim .nat) :=
...
-/
#guard_msgs (error) in
def famFoldPartial : SomeTerm recEmptySig [] (tyA ⇒ TyWf.prim .nat) :=
  ⟨.lam (.mutualRecursiveFamily_rec 0 (.var (v♯0))
...
example : (TyWf.DenRec.unfold natListSchema _ (SomeTerm.run GlobalEnv.nil natOne)).1.val = 1 :=
  rfl
