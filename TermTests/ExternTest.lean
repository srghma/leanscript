...
example : (.lean_nat_add 2 3 : Extern (.prim .nat)) = .preludeExtern (.lean_nat_add 2 3) := by kernel_rfl
...
example : isNatAdd (.lean_nat_add 2 3 : Extern (.prim .nat)) = true := by decide +kernel
example : isNatAdd (.lean_nat_sub 2 3 : Extern (.prim .nat)) = false := by decide +kernel
...
example : tyWfOf (List Nat) = TyWf.list (.prim .nat) := by kernel_rfl
example : tyWfOf (Option Char) = TyWf.option (.prim .char) := by kernel_rfl
example : tyWfOf (Float × Int) = TyWf.prod (.prim .float) (.prim .int) := by kernel_rfl
example : tyWfOf Ordering = TyWf.ordering := by kernel_rfl
example : tyOf Ordering = .enum ⟨0, -1⟩ := by kernel_rfl
