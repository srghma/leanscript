...
def fibTRTerm :=
  (.lam (.letT (.recTaggedUnion_rec 0 (.var (v♯0)) fibTRCases) (.ap (.ap (.var (v♯0)) (.nat_mk 0)) (.nat_mk 1))) :
    Term sigAdd [] _ (peanoTy ⇒ natT) .lam)
...
def fibPairTerm :=
  (.lam (.letT (.recTaggedUnion_rec 0 (.var (v♯0)) fibPairCases) (.record_casesOn (.var (v♯0)) (.var (v♯0)))) :
    Term sigAdd [] _ (peanoTy ⇒ natT) .lam)
