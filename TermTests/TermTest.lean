...
/-- `fun x => x`, at `nat ⇒ nat`. -/
def idNat : Term emptySig [] 0 (TyWf.prim .nat ⇒ TyWf.prim .nat) .UNKNOWN := .lam (.var (v♯0))

/-- `f 3`, for a function `f` that is a variable: an application whose function is not
    a `fun`. -/
def applyTo3 : Term emptySig [] 0 ((TyWf.prim .nat ⇒ TyWf.prim .nat) ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.ap (.var (v♯0)) (.nat_mk 3))

/-- `fun x => fun y => x`: a curried constant function. -/
def constNat : Term emptySig [] 0 (TyWf.prim .nat ⇒ TyWf.prim .bool ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.lam (.var (v♯1)))

/-- `fun n => let x = n + n; x + x`: a `let` of a computation whose variable is used
    twice — the one `let` the grammar keeps. -/
def letTwice : Term emptySig [] 0 (TyWf.prim .nat ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.letE
...
def callDouble : Term doubleSig [] 0 (TyWf.prim .nat) .UNKNOWN :=
  .ap (.global .here) (.nat_mk 21)

/-! ## Literals -/

/-- A bit-vector literal: the positivity of the width is written by `by decide`. -/
def bv : Term emptySig [] 0 (TyWf.prim (.bitvec 8)) .UNKNOWN := .bitvec_mk (v := 7#8)

/-- A string literal. -/
def hello : Term emptySig [] 0 (TyWf.prim .string) .UNKNOWN := .string_mk "hello.term"

/-- A character literal. -/
def theLetterA : Term emptySig [] 0 (TyWf.prim .char) .UNKNOWN := .char_mk 'a'

/-- A 64-bit float literal. -/
def half : Term emptySig [] 0 (TyWf.prim .float) .UNKNOWN := .float_mk 0.5

/-! ## Eliminators of the terminal types -/

/-- `if b then 1 else 0`, as a function of `b`. -/
def boolToNat : Term emptySig [] 0 (TyWf.prim .bool ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.bool_casesOn (.var (v♯0)) (.nat_mk 1) (.nat_mk 0))

/-- `fun n => match n with | 0 => 0 | k + 1 => k`: the predecessor, by case analysis. -/
def pred : Term emptySig [] 0 (TyWf.prim .nat ⇒ TyWf.prim .nat) .UNKNOWN :=
...
def foldNat : Term emptySig [] 0 (TyWf.prim .nat ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.nat_rec 0 (.var (v♯0)) (.cons (.nat_mk 0) .nil) (.var (v♯1)))

/-- The code point of a character, as a `uint32`. -/
def charCode : Term emptySig [] 0 (TyWf.prim .char ⇒ TyWf.prim .uint32) .UNKNOWN :=
  .lam (.char_casesOn (.var (v♯0)) (.var (v♯0)))

/-- The byte index of an unchecked position. -/
def rawByteIdx : Term emptySig [] 0 (TyWf.prim .stringPosRaw ⇒ TyWf.prim .nat) .UNKNOWN :=
...
def substringStr : Term emptySig [] 0 (TyWf.prim .substringRaw ⇒ TyWf.prim .string) .UNKNOWN :=
  .lam (.substringRaw_casesOn (.var (v♯0)) (.var (v♯0)))

/-! ## Delays, and arrays -/

/-- A memoised delay of `3`. -/
def thunkedThree : Term emptySig [] 0 (TyWf.thunk (TyWf.prim .nat)) .UNKNOWN :=
  .thunk_mk (.nat_mk 3)

/-- Forcing a memoised delay given as an argument. -/
def forceThunk : Term emptySig [] 0 (TyWf.thunk (TyWf.prim .nat) ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.thunk_force (.var (v♯0)))

/-- An unmemoised delay of `3`. -/
def lazyThree : Term emptySig [] 0 (TyWf.lazy (TyWf.prim .nat)) .UNKNOWN :=
  .lazy_mk (.nat_mk 3)

/-- The array `#[1, 2, 3]`. -/
def oneTwoThree : Term emptySig [] 0 (TyWf.array (TyWf.prim .nat)) .UNKNOWN :=
...
def headOrZero : Term emptySig [] 0 (TyWf.array (TyWf.prim .nat) ⇒ TyWf.prim .nat) .UNKNOWN :=
...
def foldArray : Term emptySig [] 0 (TyWf.array (TyWf.prim .nat) ⇒ TyWf.prim .nat) .UNKNOWN :=
...
def middle : Term emptySig [] 0 (TyWf.enum three) .UNKNOWN := .enum_mk three ⟨1, by decide⟩
...
/-- A dispatch on `three`: one branch per constructor, and no default.  The branches
    ...
    at minimum. -/
def enumToNat : Term emptySig [] 0 (TyWf.enum three ⇒ TyWf.prim .nat) .UNKNOWN :=
...
/-- Its dispatch: a branch for each of the two extra constructors, and then the three an
    enum always has. -/
def fiveToNat : Term emptySig [] 0 (TyWf.enum five ⇒ TyWf.prim .nat) .UNKNOWN :=
...
/-- The record `(3, true)`. -/
def pair : Term emptySig [] 0 (TyWf.record pairSchema) .UNKNOWN :=
...
/-- The first field of such a record: the eliminator binds both fields, and the first is
    index `0`. -/
def pairFst : Term emptySig [] 0 (TyWf.record pairSchema ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.record_casesOn (.var (v♯0)) (.var (v♯0)))

/-- Its second field, at index `1`. -/
def pairSnd : Term emptySig [] 0 (TyWf.record pairSchema ⇒ TyWf.prim .bool) .UNKNOWN :=
...
def someThree : Term emptySig [] 0 (TyWf.taggedUnion optNat) .UNKNOWN :=
...
def noneNat : Term emptySig [] 0 (TyWf.taggedUnion optNat) .UNKNOWN :=
...
def optNatOrZero : Term emptySig [] 0 (TyWf.taggedUnion optNat ⇒ TyWf.prim .nat) .UNKNOWN :=
...
def optNatOrZeroWithDefault : Term emptySig [] 0 (TyWf.taggedUnion optNat ⇒ TyWf.prim .nat) .UNKNOWN :=
...
def natOrNothingToNat : Term emptySig [] 0 (TyWf.taggedUnion natOrNothing ⇒ TyWf.prim .nat) .UNKNOWN :=
...
def enumLastOrZero : Term emptySig [] 0 (TyWf.enum three ⇒ TyWf.prim .nat) .UNKNOWN :=
...
/-- Two of the five constructors, named smallest first; the other three take the
    default. -/
def fiveTwoOrZero : Term emptySig [] 0 (TyWf.enum five ⇒ TyWf.prim .nat) .UNKNOWN :=
...
-/
#guard_msgs (error) in
def fiveOutOfOrder : Term emptySig [] 0 (TyWf.enum five ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.enum_casesOnWithDefault (.var (v♯0))
...
-/
#guard_msgs (error) in
def fiveRepeated : Term emptySig [] 0 (TyWf.enum five ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.enum_casesOnWithDefault (.var (v♯0))
...
def natBoolNatTwoOrZero :
    Term emptySig [] 0 (TyWf.taggedUnion natBoolNat ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.taggedUnion_casesOnWithDefault (.var (v♯0))
...
-/
#guard_msgs (error) in
def threeAllNamed : Term emptySig [] 0 (TyWf.enum three ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.enum_casesOnWithDefault (.var (v♯0))
...
-/
#guard_msgs (error) in
def optNatAllNamed : Term emptySig [] 0 (TyWf.taggedUnion optNat ⇒ TyWf.prim .nat) .UNKNOWN :=
  .lam (.taggedUnion_casesOnWithDefault (.var (v♯0))
...
-/
#guard_msgs (error) in
def atOccurrenceLeaf : Term emptySig [] 0 (Ty.self.toTyWf) .UNKNOWN := .nat_mk 0
...
-/
#guard_msgs (error) in
def atNonPositiveBinder : Term emptySig [] 0 ((Ty.recTaggedUnion (.payloadFirst ⟨.fn .self (.prim .nat), []⟩ [] [])).toTyWf) .UNKNOWN :=
  .nat_mk 0
...
-/
#guard_msgs (error) in
def idNatAt3 : Term emptySig [] 0 (TyWf.prim .nat) .UNKNOWN :=
...
-/
#guard_msgs (error) in
def letThree : Term emptySig [] 0 (TyWf.prim .nat) .UNKNOWN :=
...
-/
#guard_msgs (error) in
def letOnce : Term emptySig [] 0 (TyWf.prim .nat ⇒ TyWf.prim .nat) .UNKNOWN :=
...
-/
#guard_msgs (error) in
def ifTrue : Term emptySig [] 0 (TyWf.prim .nat) .UNKNOWN :=
...
-/
#guard_msgs (error) in
def thunkedThreeForced : Term emptySig [] 0 (TyWf.prim .nat) .UNKNOWN :=
...
-/
#guard_msgs (error) in
def pairFstOfPair : Term emptySig [] 0 (TyWf.prim .nat) .UNKNOWN :=
...
-/
#guard_msgs (error) in
def onePlusTwo : Term emptySig [] 0 (TyWf.prim .nat) .UNKNOWN :=
