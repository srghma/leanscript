...
Step 3 (second half) of design **N** of `proposals/NominalTyProposal.md` (§2.4).
...
The leaves (`LeanPrimTy.two`) are settled one by one; the two float types have no decidable
equality, and their two values are told apart with `native_decide`.
-/
...
theorem float_zero_beq_zero : ((0.0 : Float) == 0.0) = true := by native_decide
theorem float_one_beq_zero : ((1.0 : Float) == 0.0) = false := by native_decide
theorem float32_zero_beq_zero : ((0.0 : Float32) == 0.0) = true := by native_decide
theorem float32_one_beq_zero : ((1.0 : Float32) == 0.0) = false := by native_decide
...
def LeanPrimTy.two : (p : LeanPrimTy) → p.Nondeg = true → Two p.denote
  ...
  | .stringPos s, h =>
      ⟨s.startPos, s.endPos, fun q => q.offset.byteIdx == 0, rfl, by
        have : s.utf8ByteSize ≠ 0 := fun e => by
          simp [LeanPrimTy.Nondeg] at h; exact h (String.utf8ByteSize_eq_zero_iff.mp e)
  ...
  | .float, _ => ⟨0.0, 1.0, fun f => f == 0.0, float_zero_beq_zero, float_one_beq_zero⟩
  | .float32, _ => ⟨0.0, 1.0, fun f => f == 0.0, float32_zero_beq_zero, float32_one_beq_zero⟩
  | .floatModel, _ =>
