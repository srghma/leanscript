...
theorem Ty.lift_lower : (t : Ty ks) → (x : Ty.den E (Ty.map f t)) →
  ...
  | .union cs, x => Ctors.lift_lower cs x
  | .data _, _ => rfl
theorem Ty.lower_lift : (t : Ty ks) → (x : Ty.den (fun r => E (f r)) t) →
  ...
  | .union cs, x => Ctors.lower_lift cs x
  | .data _, _ => rfl
...
theorem Fields.lower_lift : (fs : Fields ks) → (x : Fields.den (fun r => E (f r)) fs) →
  ...
  | .cons t fs, x => by
      show (Ty.lower f E t (Ty.lift f E t x.1), Fields.lower f E fs (Fields.lift f E fs x.2)) = x
      rw [Ty.lower_lift t, Fields.lower_lift fs]; exact rfl
theorem Ctors.lift_lower : (cs : Ctors ks) → (x : Ctors.den E (Ctors.map f cs)) →
...
theorem Ctors.lower_lift : (cs : Ctors ks) → (x : Ctors.den (fun r => E (f r)) cs) →
...
theorem BCtor.unrollTwo_rollTwo {g g' : Nat} : (c : BCtor ks n g) → (d : BCtor ks n g') →
...
theorem BCtor.unrollCons_rollCons {g : Nat} {R : IPF n} {RT : Type}
    (rest : RT → R.Obj (fun i => Ty.den E' (σ i))) (unrest : R.Obj (fun i => Ty.den E' (σ i)) → RT)
    (h : ∀ r, unrest (rest r) = r) : (c : BCtor ks n g) →
...
theorem BCtors.unroll_roll : (cs : BCtors ks n) → (x : Ctors.den E' (BCtors.inst w σ cs)) →
...
theorem Alts.unroll_roll {g : Nat} : (u : Alts ks n g) → (x : Ctors.den E' (Alts.inst w σ u)) →
...
theorem Decl.unroll_roll {g : Nat} : (d : Decl ks n g) → (x : Ty.den E' (Decl.inst w σ d)) →
  ...
  | .wrap f, x => Fld.unroll_roll w E' σ f x
  | .record f fs, x => by
      show (Fld.unroll w E' σ f (Fld.roll w E' σ f x.1), Flds.unroll w E' σ fs (Flds.roll w E' σ fs x.2)) = x
      rw [Fld.unroll_roll, Flds.unroll_roll]; exact rfl
  | .union u, x => Alts.unroll_roll w E' σ u x
...
theorem Flds.roll_unroll {g : Nat} : (fs : Flds ks n g) →
  ...
  | .cons f fs, x => by
      exact (congrArg₂ IPF.Obj.pair (Fld.roll_unroll w E' σ f x.fst)
        (Flds.roll_unroll fs x.snd)).trans (IPF.Obj.pair_fst_snd x)

theorem BCtor.rollTwo_unrollTwo {g g' : Nat} : (c : BCtor ks n g) → (d : BCtor ks n g') →
...
theorem BCtor.rollCons_unrollCons {g : Nat} {R : IPF n} {RT : Type}
    (rest : RT → R.Obj (fun i => Ty.den E' (σ i))) (unrest : R.Obj (fun i => Ty.den E' (σ i)) → RT)
    (h : ∀ o, rest (unrest o) = o) : (c : BCtor ks n g) →
...
theorem BCtors.roll_unroll : (cs : BCtors ks n) →
...
theorem Alts.roll_unroll {g : Nat} : (u : Alts ks n g) →
...
theorem Decl.roll_unroll {g : Nat} : (d : Decl ks n g) →
  ...
  | .wrap f, x => Fld.roll_unroll w E' σ f x
  | .record f fs, x => by
      exact (congrArg₂ IPF.Obj.pair (Fld.roll_unroll w E' σ f x.fst)
        (Flds.roll_unroll w E' σ fs x.snd)).trans (IPF.Obj.pair_fst_snd x)
  | .union u, x => Alts.roll_unroll w E' σ u x
