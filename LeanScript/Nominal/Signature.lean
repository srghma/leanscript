...
public meta import LeanScript.Nominal.Gen.Cache
...
of the signature, oldest first (`LeanScript.Nominal.Gen.declareBlocks`).  It adds:
...
`#leanscript_get_ty` and `#leanscript_get_ctor` (`LeanScript.Nominal.GetCtor`) translate
...
the same tree.  Everything else stays structural (see `LeanScript.Nominal.Gen.toCIR`).

Proof, instance and `Unit` fields are erased.  It refuses, with an error: a type with no
constructor, a type with one constructor and no field (unit-like), an inductive family with
indices, a field whose type is a type or depends on an earlier field, a recursive
occurrence in the domain of a function or under a `Thunk`, and a recursive SCC with no
