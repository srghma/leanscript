...
The constructor API of design **N** (§2.6 and step 6 of §2.9 of
`proposals/NominalTyProposal.md`, following §A.8 of `proposals/WTypeTyProposal.md`).  All
three are term elaborators backed by the cache (`LeanScript.Nominal.Gen.Cache`): the definition is
...
* `#leanscript_get_ctor c (x := T)…` — the constructor function of the Lean constructor `c`
  ...
  | a proof, instance or `Unit` field | none (erased) |
  ...
  * otherwise `Term.union_mk` at the constructor's position (two field-less constructors
    are the union denoting `Bool`); `Bool.true`/`Bool.false` are literals of the leaf `bool`;
...
def ensureTy (T : Expr) : TermElabM Name := do
  ...
  match prog? with
  | some p =>
    emitDef decl (← `(LeanScript.Nominal.Ty $(mkIdent (p.name ++ `ks)))) tyStx
  | none =>
    let ks := mkIdent `ks
    emitDef decl (← `(∀ {$ks : List Nat}, LeanScript.Nominal.Ty $ks)) (← `(fun {$ks} => $tyStx))
...
structure TypePlan where
  ...
  enum? : Option (Nat × Int)
  /-- Is it `Bool`, the leaf whose constructors are literals? -/
  isBool : Bool := false
...
def planType (T : Expr) (prog? : Option ProgInfo) : M TypePlan := do
  ...
  | .node n =>
    ...
    | .structural =>
      ...
      return { ctors, layout, data? := none, enum? }
...
def Frame.close (F : Frame) (fty fn : Lean.Term) : TermElabM (Lean.Term × Lean.Term) := do
  ...
  for v in F.varIds.reverse do
    fty ← `(($v : LeanScript.Nominal.Ty $(F.ksT)) → $fty)
    fn ← `(fun $v => $fn)
  let gam := mkIdent `Γ
  fty ← `(∀ {$gam : LeanScript.Nominal.Ctx $(F.ksT)}, $fty)
  fn ← `(fun {$gam} => $fn)
  if F.prog?.isNone then
    let ks := mkIdent `ks
    let d := mkIdent `Δ
    fty ← `(∀ {$ks : List Nat} {$d : LeanScript.Nominal.DSig $ks}, $fty)
    fn ← `(fun {$ks} {$d} => $fn)
  return (fty, fn)
...
def ensureCtor (id : Ident) (named : Array (Ident × Lean.Term)) : TermElabM Name := do
  ...
  withInstance cinfo.induct named fun T key vars => do
    ...
    let payload ← if plan.isBool then
        if ctor == ``Bool.true then `(LeanScript.Nominal.Term.lit LeanPrimTy.bool rfl true)
        else `(LeanScript.Nominal.Term.lit LeanPrimTy.bool rfl false)
      else ctorBodyStx plan.ctors.size pos plan.enum? args
    let body ← match plan.data? with
      | some (b, j) => `(LeanScript.Nominal.Term.data_in $(← brefStx F.c b) $(quote j) $payload)
      | none => pure payload
    -- the type and the function
    let mut fty ← `(LeanScript.Nominal.Term $(F.dT) $gam $(← plan.layout.stx F.c F.varIds))
    let mut fn := body
    for q in (List.range fields.size).reverse do
      fty ← `(($(argIds[q]!) : LeanScript.Nominal.Term $(F.dT) $gam
...
def casesBodyStx (plan : TypePlan) (scrut : Lean.Term) (bs : Array Lean.Term) :
  ...
  if plan.isBool then
    `(LeanScript.Nominal.Term.ite $scrut $(bs[1]!) $(bs[0]!))
  else if plan.enum?.isSome then
    let i := mkIdent `i
    let mut sel := bs[m - 1]!
    for p in (List.range (m - 1)).reverse do
      sel ← `(if ($i).val = $(quote p) then $(bs[p]!) else $sel)
    `(LeanScript.Nominal.Term.enum_casesOn $scrut (fun $i => $sel))
  else if m = 1 then
    if plan.ctors[0]!.2.size = 1 then `(LeanScript.Nominal.Term.letE $scrut $(bs[0]!))
    else `(LeanScript.Nominal.Term.record_casesOn $scrut $(bs[0]!))
  else
    let mut br ← `(LeanScript.Nominal.Branches.two $(bs[m - 2]!) $(bs[m - 1]!))
    for p in (List.range (m - 2)).reverse do
      br ← `(LeanScript.Nominal.Branches.cons $(bs[p]!) $br)
    `(LeanScript.Nominal.Term.union_casesOn $scrut $br)
...
def ensureCases (id : Ident) (named : Array (Ident × Lean.Term)) : TermElabM Name := do
  ...
  withInstance ind named fun T key vars => do
    ...
    if let some d ← cached? gkey then return d
    let decl ← F.declName ind (.mkSimple ind.lastComponentAsString) (if F.prog?.isSome then `cases else `leanScriptCases)
    let gam := mkIdent `Γ
    let tau := mkIdent `τ
    let x := mkIdent `scrut
    let brIds : Array Ident := plan.ctors.map fun (c, _) =>
      mkIdent (.mkSimple ("on_" ++ c.lastComponentAsString))
    let scrut ← match plan.data? with
      | some (b, j) => `(LeanScript.Nominal.Term.data_out $(← brefStx F.c b) $(quote j) $x)
      | none => pure x
    let body ← casesBodyStx plan scrut (brIds.map fun b => ⟨b.raw⟩)
    let mut fty ← `(LeanScript.Nominal.Term $(F.dT) $gam $tau)
    let mut fn := body
    for p in (List.range plan.ctors.size).reverse do
      ...
      fty ← `(($(brIds[p]!) : LeanScript.Nominal.Term $(F.dT) $ctx $tau) → $fty)
      fn ← `(fun $(brIds[p]!) => $fn)
    fty ← `(($x : LeanScript.Nominal.Term $(F.dT) $gam $(← plan.layout.stx F.c F.varIds)) → $fty)
    fn ← `(fun $x => $fn)
    fty ← `(∀ {$tau : LeanScript.Nominal.Ty $(F.ksT)}, $fty)
    fn ← `(fun {$tau} => $fn)
    let (fty', fn') ← F.close fty fn
    emitDef decl fty' fn'
    addEntry (.gen gkey decl)
