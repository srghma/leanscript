...
public meta import LeanScript.Nominal.Gen.Translate
...
`LeanScript.Nominal.Ty`, `Fld`, `Decl`, … and of the term formers.  Every identifier is
written in a quotation inside `LeanScript.Nominal`, so it is resolved here and the syntax can
...
def ctorBodyStx (m p : Nat) (enum : Option (Nat × Int)) (args : List Lean.Term) :
  ...
  if m = 1 then
    match args with
    | [a] => return a
    | _ => `(LeanScript.Nominal.Term.record_mk $(← argsStx args))
  else if let some (n, s) := enum then
    `(LeanScript.Nominal.Term.enum_mk ⟨$(quote (n - 3)), $(← intStx s)⟩ ⟨$(quote p), by decide⟩)
  else
    `(LeanScript.Nominal.Term.union_mk $(← ctorIxStx m p) $(← argsStx args))
