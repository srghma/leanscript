...
public meta import LeanScript.Nominal.Gen.Read
...
* one constructor of one field is that field (the wrapper is erased), of two or more fields
  a record;
* two or more constructors are a union, or an enum when there are three or more and none has
  a field (two field-less constructors are the union denoting `Bool`).
  ...
  | .node n =>
    ...
    | .structural =>
      ...
      let t ← match node.ctors.size with
        ...
        | m =>
          if node.ctors.all (·.2.isEmpty) && 3 ≤ m then
            pure (.enum m (enumShift n.getAppFn.constName!))
...
partial def toFIR (members : Array Nat) (e : Expr) : M FIR := do
  ...
  | .thunk _ =>
    fail m!"a recursive occurrence under `Thunk` is not supported{indentExpr e}"
