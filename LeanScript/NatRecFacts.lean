...
/-- The value of the node — here answering with the term's own value, `Dest.ret` — **is**
    the fold: its base values are the atoms `base`, and its step runs the branch with the
    window in front of the environment. -/
