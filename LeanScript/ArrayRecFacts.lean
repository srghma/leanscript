...
* `Term.eval_array_rec`, `Term.eval_array_rec_base` and `Term.eval_array_rec_step` — the
  same facts for the grammar's node, stated of `Term.eval`.  An array of the language
  denotes a Lean `Array`; the node folds the list of its elements (`Array.toList`), and
  gives its branch the tail as an array again (`List.toArray`).
-/
...
variable {Sg : Sig} {Γ : Ctx} {σ : TyWf} (G : GlobalEnv Sg.decls) {u ub : Usage Γ}
    ...
    (hRec : 0 < Usage.sumN τ (k + 1) (Usage.tail (Usage.tail w))) (hStep : k = 0 → kb ≠ .var)
    (hClosed : Head.closedComp
      (u + ub + Usage.many (Usage.dropN τ (k + 1) (Usage.tail (Usage.tail w)))) τ .comp = false)
    (env : Env Γ) (h : Term.NoRecMk (Term.array_rec k arr bases branch hRec hStep hClosed))
...
theorem Term.eval_array_rec :
    Term.eval G (Term.array_rec k arr bases branch hRec hStep hClosed) env h =
