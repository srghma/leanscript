...
variable {Sg : Sig} {Γ : Ctx} {σ : TyWf} (G : GlobalEnv Sg.decls) {u ub : Usage Γ}
    ...
    (hRec : 0 < Usage.sumN τ (k + 1) (Usage.tail (Usage.tail w)))
    (env : Env Γ) (h : Term.NoRecMk (Term.array_rec k arr bases branch hRec))
...
theorem Term.eval_array_rec :
    Term.eval G (Term.array_rec k arr bases branch hRec) env h =
