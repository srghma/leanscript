...
δ-rule of `LeanScript.Reduce` that runs a one-argument extern carries no side condition.
The entries that denoted no function of their argument's value are commented out of
`LeanScript.LeanInitPureExterns`, each with its reason; see `EXTERN_EVALUATION.md`.
-/
