...
| `while c do …`, `repeat …` and `repeat … until c` in `Id` (with `break`, `continue`, `return` from inside, and nested in other loops) | `while_loop`: the initial state and the body read as the step `ForInStep β`; its meaning is Lean's loop whenever the loop stops within `2 ^ 64` iterations (`LeanScript.loop_forIn_eq_whileIter`) — see `TermTests/ToTermTest/While.lean` |
