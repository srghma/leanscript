...
| `while c do …`, `repeat …` and `repeat … until c` in `Id` (with `break`, `continue`, `return` from inside, and nested in other loops), **only when it is a structural recursion**: a `Nat` variable of the state goes down by one (`x - 1` under a test `x ≠ 0`, or `n` in the case `n + 1` of a `match` on `x`) or up by one (`x + 1` under a test `x < b` / `x ≤ b`, `b` not changed by the loop) on every path that goes on | `nat_rec` on the number of iterations the counter allows plus one, folding the step `ForInStep β` (`LeanScript.ToTerm.whileAsNatRec`; the equation is `LeanScript.loop_forIn_eq_natRec`, for every input). Any other loop is **rejected**: there is no fuel and no measure — see `TermTests/ToTermTest/While.lean` |
