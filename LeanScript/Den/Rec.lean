...
(`LeanScript.Ty.Cont`): a node is a constructor number with that constructor's fields
...
* `recBindEnv`: the environment a branch of the fold binds, read off a node whose
  subtrees carry the fold's answers (`LeanScript.WTree.memo`).
-/
...
/-- A value of the tree `a` unfolded at `R` is a shape of `a` with a value of `R` in each
    hole. -/
def roll : (a : Ty) → Ty.Den (substOcc R .familyMember a) → (Ty.Cont a).Ext (Ty.Den R)
...
def rollAt : (l : LeanTaggedUnionSchema Ty) → (t : Nat) →
    Ty.DenAt (substOccTU R .familyMember l) t → (Ty.ContAt l t).Ext (Ty.Den R)
...
def rollAtCP : (c : CtorsWithPayload Ty) → (t : Nat) →
    Ty.DenAtCP (substOccCP R .familyMember c) t → (Ty.ContAtCP c t).Ext (Ty.Den R)
...
def rollAtList : (cs : List (List Ty)) → (t : Nat) →
    Ty.DenAtList (substOccCtors R .familyMember cs) t → (Ty.ContAtList cs t).Ext (Ty.Den R)
  | [], _, v => PEmpty.elim v
...
def unroll : (a : Ty) → (Ty.Cont a).Ext (Ty.Den R) → Ty.Den (substOcc R .familyMember a)
...
/-- `Ty.unroll`, on a node. -/
def unrollShape : (s : TyShape Ty) → (Ty.ContShape s).Ext (Ty.Den R) →
...
def unrollList : (ts : List Ty) → (Ty.ContList ts).Ext (Ty.Den R) →
...
def unrollAt : (l : LeanTaggedUnionSchema Ty) → (t : Nat) →
    (Ty.ContAt l t).Ext (Ty.Den R) → Ty.DenAt (substOccTU R .familyMember l) t
...
def unrollAtCP : (c : CtorsWithPayload Ty) → (t : Nat) →
    (Ty.ContAtCP c t).Ext (Ty.Den R) → Ty.DenAtCP (substOccCP R .familyMember c) t
...
def unrollAtList : (cs : List (List Ty)) → (t : Nat) →
    (Ty.ContAtList cs t).Ext (Ty.Den R) → Ty.DenAtList (substOccCtors R .familyMember cs) t
  | [], _, x => PEmpty.elim x.1
...
example (L : LeanTaggedUnionSchema Ty) :
    Ty.Den (.recTaggedUnion L) = WTree (RecNode L) (RecHole L) := rfl
...
def DenRec.mk (L : LeanTaggedUnionSchema Ty) (v : Ty.DenTU (recUnfoldTy L)) :
  ...
  WTree.mk ⟨⟨v.1.val, length_substOccTU _ _ L ▸ v.1.isLt⟩, r.1⟩ r.2
...
theorem roll_unrollShape : ∀ (s : TyShape Ty) (x : (Ty.ContShape s).Ext (Ty.Den R)),
  ...
  | .record ⟨a, b, rest⟩, x => by
      revert x
      show ∀ x : (Cont.prod (Ty.Cont a) (Cont.prod (Ty.Cont b) (Ty.ContList rest))).Ext
          (Ty.Den R),
        Cont.Ext.pair (roll R a (unroll R a x.fst))
          (Cont.Ext.pair (roll R b (unroll R b x.snd.fst))
            (rollList R rest (unrollList R rest x.snd.snd))) = x
      intro x
  ...
  | .taggedUnion l, x => by
      ...
      exact congrArg (fun r : (Ty.ContAt l t.val).Ext (Ty.Den R) =>
        (⟨⟨t, r.1⟩, r.2⟩ :
          (Cont.sigma (Fin l.length) (fun t => Ty.ContAt l t.val)).Ext (Ty.Den R)))
...
theorem roll_unrollAt : ∀ (l : LeanTaggedUnionSchema Ty) (t : Nat)
    (x : (Ty.ContAt l t).Ext (Ty.Den R)), rollAt R l t (unrollAt R l t x) = x
...
theorem roll_unrollAtCP : ∀ (c : CtorsWithPayload Ty) (t : Nat)
    (x : (Ty.ContAtCP c t).Ext (Ty.Den R)), rollAtCP R c t (unrollAtCP R c t x) = x
  | .here ⟨a, as⟩ _, 0, x => by
      revert x
      show ∀ x : (Cont.prod (Ty.Cont a) (Ty.ContList as)).Ext (Ty.Den R),
        Cont.Ext.pair (roll R a (unroll R a x.fst)) (rollList R as (unrollList R as x.snd)) = x
...
theorem roll_unrollAtList : ∀ (cs : List (List Ty)) (t : Nat)
    (x : (Ty.ContAtList cs t).Ext (Ty.Den R)), rollAtList R cs t (unrollAtList R cs t x) = x
  | [], _, x => PEmpty.elim x.1
...
theorem DenRec.mk_unfold (L : LeanTaggedUnionSchema Ty) (v : Ty.Den (.recTaggedUnion L)) :
  ...
  obtain ⟨⟨⟨t, ht⟩, s⟩, f⟩ := v
  have key : ∀ r : (Ty.ContAt L t).Ext (Ty.Den (.recTaggedUnion L)), r = ⟨s, f⟩ →
      (WTree.mk ⟨⟨t, ht⟩, r.1⟩ r.2 : Ty.Den (.recTaggedUnion L)) = WTree.mk ⟨⟨t, ht⟩, s⟩ f := by
    rintro r rfl; rfl
...
`LeanScript.Term.recTaggedUnion_rec` folds a value by `LeanScript.WTree.memo`: the answer
...
abbrev RecMemo (l : LeanTaggedUnionSchema (TyWfIn 1)) (τ : TyWf) : Type :=
  Memo (Ty.RecNode (recL l)) (Ty.RecHole (recL l)) (TyWf.Den τ)
...
abbrev RecFields (l : LeanTaggedUnionSchema (TyWfIn 1)) (τ : TyWf) (fs : List (TyWfIn 1)) :
    Type :=
  (Ty.ContList (fs.map TyWfIn.toTy)).Ext (RecMemo l τ)
...
def recBindField (l : LeanTaggedUnionSchema (TyWfIn 1)) (hwf : Ty.Wf (TyWf.recTaggedUnionTy l))
    (τ : TyWf) (a : TyWfIn 1) (e : (Ty.Cont a.toTy).Ext (RecMemo l τ)) :
...
def recBindEnv (l : LeanTaggedUnionSchema (TyWfIn 1)) (hwf : Ty.Wf (TyWf.recTaggedUnionTy l))
  ...
  | [], _ => PUnit.unit
  | ⟨.self, _⟩ :: fs, e =>
      let m := e.fst.2 PUnit.unit
      (m.tree, m.answer, recBindEnv l hwf τ fs e.snd)
  | ⟨.familyMember i, h⟩ :: fs, e =>
      (recBindField l hwf τ ⟨.familyMember i, h⟩ e.fst, recBindEnv l hwf τ fs e.snd)
  | ⟨.shape sh, h⟩ :: fs, e =>
      (recBindField l hwf τ ⟨.shape sh, h⟩ e.fst, recBindEnv l hwf τ fs e.snd)
  | ⟨.recTaggedUnion l', h⟩ :: fs, e =>
      (recBindField l hwf τ ⟨.recTaggedUnion l', h⟩ e.fst, recBindEnv l hwf τ fs e.snd)
  | ⟨.recObject r, h⟩ :: fs, e =>
      (recBindField l hwf τ ⟨.recObject r, h⟩ e.fst, recBindEnv l hwf τ fs e.snd)
  | ⟨.recAlias b, h⟩ :: fs, e =>
      (recBindField l hwf τ ⟨.recAlias b, h⟩ e.fst, recBindEnv l hwf τ fs e.snd)
  | ⟨.mutualRecursiveFamily f, h⟩ :: fs, e =>
      (recBindField l hwf τ ⟨.mutualRecursiveFamily f, h⟩ e.fst, recBindEnv l hwf τ fs e.snd)

/-- The hole of a field that is `Ty.self`. -/
def selfHole : (a : Ty) → a = .self → (s : (Ty.Cont a).S) → (Ty.Cont a).P s
  | .self, _, _ => PUnit.unit
...
def selfFieldMemo {l : LeanTaggedUnionSchema (TyWfIn 1)} {τ : TyWf} :
  ...
  | a :: _, .here h, e => e.fst.2 (selfHole a.toTy h e.fst.1)
  | _ :: _, .there sf, e => selfFieldMemo sf e.snd
