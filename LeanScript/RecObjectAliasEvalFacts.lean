...
theorem Term.eval_recObject_mk_unfold {Γ : Ctx} (fs : LeanRecordSchema (TyWfIn 1))
    ...
    (h : Term.NoRecMk (.recObject_mk fs hwf fields)) :
    TyWf.DenObj.unfold fs hwf (Term.eval G (.recObject_mk fs hwf fields) env h) =
      Spine.eval G fields env h :=
...
theorem Term.eval_recObject_casesOn_mk {Γ : Ctx} {τ : TyWf}
    ...
    (h : Term.NoRecMk (.recObject_casesOn (.recObject_mk fs hwf fields) body)) :
    Term.eval G (.recObject_casesOn (.recObject_mk fs hwf fields) body) env h =
      Term.eval G body (Env.append (Spine.eval G fields env h.1) env) h.2 := by
  show Term.eval G body
      (Env.append (TyWf.DenObj.unfold fs hwf (TyWf.DenObj.mk fs hwf _)) env) h.2 = _
...
theorem Term.eval_recAlias_mk_unfold {Γ : Ctx} (b : TyWfIn 1)
    ...
    (env : Env Γ) (h : Term.NoRecMk (.recAlias_mk b hwf value)) :
    TyWf.DenAlias.unfold b hwf (Term.eval G (.recAlias_mk b hwf value) env h) =
      Term.eval G value env h :=
...
theorem Term.eval_recAlias_casesOn_mk {Γ : Ctx} {τ : TyWf} (b : TyWfIn 1)
    ...
    (h : Term.NoRecMk (.recAlias_casesOn (.recAlias_mk b hwf value) body)) :
    Term.eval G (.recAlias_casesOn (.recAlias_mk b hwf value) body) env h =
      Term.eval G body (Term.eval G value env h.1, env) h.2 := by
  show Term.eval G body
      (TyWf.DenAlias.unfold b hwf (TyWf.DenAlias.mk b hwf _), env) h.2 = _
